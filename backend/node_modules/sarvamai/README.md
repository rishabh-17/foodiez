...
The Sarvam TypeScript library provides convenient access to the Sarvam APIs from TypeScript.
...
```typescript
import { TextClient } from 'sarvamai/text';
...
```typescript
const { data, rawResponse } = await client.text.translate(...).withRawResponse();
...
```typescript
import { SarvamAIClient, logging } from "sarvamai";
